...
public class WndCatalogus3 extends WndTabbed {
    ...
    public WndCatalogus3() {
        ...
        boolean showPotions = WndCatalogus3.showPotions;
        Tab[] tabs = {
                new LabeledTab( "食物" ) {
    ...
    private void updateList() {
        ...
        float pos = 0;
        for (Class<? extends Item> itemClass : showPotions ? RingThree.getKnown() : RingTwo.getKnown()) {
        ...
        for (Class<? extends Item> itemClass : showPotions ? RingTwo.getUnknown() : RingThree.getUnknown()) {
            WndCatalogus.ListItem item = new WndCatalogus.ListItem( itemClass );
            item.setRect( 0, pos, width, ITEM_HEIGHT );
            content.add( item );
            items.add( item );
