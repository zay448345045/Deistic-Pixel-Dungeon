...
public class WndCatalogus4 extends WndTabbed {
    ...
    public WndCatalogus4() {

        super();

        resize( WIDTH, HEIGHT );

        btnJournal = new RedButton("图鉴P2", 9 ){
            @Override
        ...
        SbtnTitle = new RedButton( "图鉴P4", 9 ){
            @Override
            protected void onClick() {
                hide();
        ...
        boolean showPotions = WndCatalogus4.showPotions;
        WndTabbed.Tab[] tabs = {
                new LabeledTab( "神器" ) {
                    protected void select( boolean value ) {
                        super.select( value );
                        //WndCatalogus3.showPotions = value;
                        updateList();
                    };
                },
        };
        for (WndTabbed.Tab tab : tabs) {
            add( tab );
        }
    ...
    private void updateList() {
        ...
        float pos = 0;
        for (Class<? extends Item> itemClass : showPotions ? RingFour.getKnown() : RingFour.getKnown()) {
        ...
        for (Class<? extends Item> itemClass : showPotions ? RingFour.getUnknown() : RingFour.getUnknown()) {
            WndCatalogus.ListItem item = new WndCatalogus.ListItem( itemClass );
            item.setRect( 0, pos, width, ITEM_HEIGHT );
            content.add( item );
            items.add( item );
