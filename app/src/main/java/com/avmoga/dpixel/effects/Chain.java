 ...
 * 
 * Shamelessly stolen from Evan by Matthew Weidenhamer
 *
...
public class Chain extends Group {
	...
	public Chain(int from, int to, Callback callback){
		this(DungeonTilemap.tileCenterToWorld(from),
				DungeonTilemap.tileCenterToWorld(to),
				callback);
	}

	public Chain(PointF from, PointF to, Callback callback){
		super();
	 ...
	 rotation = (float) (Math.atan2(dy, dx) * A);

		numChains = Math.round(distance/6f)+1;

		chains = new Image[numChains];
		for (int i = 0; i < chains.length; i++){
			chains[i] = new Image(Effects.get(Effects.Type.CHAINS));
